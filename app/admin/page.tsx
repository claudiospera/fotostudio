    ...
    <div style={{ background: BG, minHeight: '100vh', color: TX, fontFamily: "'DM Sans', sans-serif", paddingBottom: selCount ? 80 : 0 }}>
      ...
      <div style={{ padding: '20px 32px' }}>
        {loading ? (
          ...
          const isOpen  = expanded.has(w.slug)
          const allSel  = w.photos.length > 0 && w.photos.every(u => selected.has(u))
          const someSel = w.photos.some(u => selected.has(u))
          const selInW  = w.photos.filter(u => selected.has(u)).length
          return (
            <section key={w.slug} style={{ marginBottom: 8, borderRadius: 8, background: S1, overflow: 'hidden' }}>
              {/* ── Header collassabile ── */}
              <div
                onClick={() => toggleExpand(w.slug)}
                style={{
              ...
              >
                <span style={{ color: T3, fontSize: 13, transition: 'transform 0.15s', display: 'inline-block', transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}>▶</span>
                <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 14, margin: 0, color: TX, flexGrow: 1 }}>
                  {w.title}
                  {selInW > 0 && <span style={{ marginLeft: 8, color: RED, fontSize: 11 }}>({selInW} sel.)</span>}
                </h2>
                {w.location && <span style={{ color: T3, fontSize: 11 }}>{w.location}</span>}
                {w.date     && <span style={{ color: T3, fontSize: 11 }}>· {w.date}</span>}
                <span style={{ color: T3, fontSize: 11 }}>{w.photos.length} foto</span>
                {isOpen && (
                  <button
                    onClick={e => { e.stopPropagation(); toggleAll(w.photos) }}
                    style={{
                  ...
                  {w.photos.map(url => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isCover && (
                          <span style={{ position: 'absolute', top: 3, left: 3, background: AMBER, color: '#1a1612', fontSize: 7, fontWeight: 700, padding: '1px 4px', borderRadius: 3 }}>C</span>
                        )}
                        {isSel && (
                          <div style={{ position: 'absolute', inset: 0, background: 'rgba(217,112,112,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
