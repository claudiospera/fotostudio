...
// app/shop/gadget/salvadanaio/page.tsx
// Salvadanaio in ceramica — foto + testo overlay
...
export default function SalvadanaiPage() {
  ...
  const [textPos,   setTextPos]   = useState({ x: 0.5, y: 0.78 })
  ...
  // Carica Google Fonts
  useEffect(() => {
    if (document.getElementById('gf-salvadanaio')) return
    const link = document.createElement('link')
    link.id   = 'gf-salvadanaio'
    link.rel  = 'stylesheet'
  ...
  const textStateRef = useRef({ textPos: { x: 0.5, y: 0.78 } })
  ...
  // ─── Add to cart con canvas render (foto + testo) ─────────────────────────
  async function handleAddToCart() {
    if (uploading || isRendering) return
    let imageUrl = uploadedUrl ?? photoUrl ?? '/images/shop/gadget/salvadanaio.png'
      ...
      if (ctx) {
        ...
        if (blob) {
          const res = await fetch('/api/shop/presign-photo', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ filename: photoFilename ?? 'salvadanaio.jpg', contentType: 'image/jpeg' }),
          })
          if (res.ok) {
            const { uploadUrl, publicUrl } = await res.json()
    ...
    <div style={{ fontFamily: 'Montserrat, sans-serif', background: '#f9f9f9', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e8e8e8', padding: '12px clamp(20px, 5vw, 60px)' }}>
        <nav style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '12px', color: '#999', maxWidth: 1140, margin: '0 auto' }}>
          ...
          <span>/</span>
          <span style={{ color: '#0a0a0a', fontWeight: 600 }}>Salvadanaio Personalizzato</span>
        </nav>
      </div>
        ...
        <div className="shop-sticky" style={{ position: 'sticky', top: 88 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ borderRadius: 16, overflow: 'hidden', background: '#f0f0f0', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 220 }}>
              <Image
                src="/images/shop/gadget/salvadanaio.png"
            ...
            <>
              ...
              {photoUrl && (
                <div style={{ padding: '14px 16px', background: '#f0f9ff', borderRadius: 12, border: '1px solid #c8e8f5', fontSize: '12px', color: '#555', lineHeight: 1.7 }}>
                  <b>💡 Suggerimento:</b> regola lo zoom e trascina la foto per centrarla nell&apos;area di stampa.
                </div>
              )}
            </>
          )}
            ...
            <>
              ...
              {text.trim() && (
                <div style={{ padding: '14px 16px', background: '#f0f9ff', borderRadius: 12, border: '1px solid #c8e8f5', fontSize: '12px', color: '#555' }}>
                  <b>💡</b> Trascina il testo nell&apos;anteprima per riposizionarlo sul salvadanaio.
                </div>
              )}
            </>
          )}
