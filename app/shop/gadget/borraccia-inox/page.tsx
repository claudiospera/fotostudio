...
// app/shop/gadget/borraccia-inox/page.tsx
// Borraccia Inox Sport — area stampa 22×14 cm — foto + testo overlay
...
const PREV_H = Math.round(PREV_W * 14 / 22)        // 255px
const OUT_W  = 2200                                 // canvas output (100px/cm × 22cm)
const OUT_H  = 1400                                 // canvas output (100px/cm × 14cm)
...
export default function BorracciaSportPage() {
  ...
  // Carica Google Fonts
  useEffect(() => {
    if (document.getElementById('gf-bor-sport')) return
    const link = document.createElement('link')
    link.id   = 'gf-bor-sport'
    link.rel  = 'stylesheet'
  ...
  async function handleAddToCart() {
    ...
    let imageUrl = uploadedUrl ?? photoUrl ?? '/images/shop/gadget/borraccia-inox.png'
      ...
      if (ctx) {
        ...
        if (blob) {
          const res = await fetch('/api/shop/presign-photo', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ filename: photoFilename ?? 'borraccia-sport.jpg', contentType: 'image/jpeg' }),
          })
          if (res.ok) {
            const { uploadUrl, publicUrl } = await res.json()
    ...
    addItem({
      productId:    'borraccia-inox',
      variantId:    'bor-inox-600',
      quantity:     qty,
      productName:  'Borraccia Inox Sport',
      variantLabel: `600ml — 22×14 cm${text.trim() ? ` · Testo: "${text.slice(0, 20)}"` : ''}`,
      price:        PRICE,
      image:        imageUrl,
    ...
    <div style={{ fontFamily: 'Montserrat, sans-serif', background: '#f9f9f9', minHeight: '100vh' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e8e8e8', padding: '12px clamp(20px, 5vw, 60px)' }}>
        <nav style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '12px', color: '#999', maxWidth: 1140, margin: '0 auto' }}>
          ...
          <span>/</span>
          <span style={{ color: '#0a0a0a', fontWeight: 600 }}>Borraccia Inox Sport</span>
        </nav>
      </div>
        ...
        <div className="shop-sticky" style={{ position: 'sticky', top: 88 }}>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ borderRadius: 16, overflow: 'hidden', background: '#f0f0f0', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 220 }}>
              <Image
                src="/images/shop/gadget/borraccia-inox.png"
                alt="Borraccia Inox Sport"
          ...
          <p style={{ fontSize: '11px', fontWeight: 700, color: '#999', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 10 }}>
            Anteprima area di stampa (22×14 cm)
          </p>
          ...
          <div style={{ position: 'relative', display: 'inline-block', width: '100%' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
              ...
              <span style={{ fontSize: '10px', color: '#bbb' }}>14 cm</span>
            </div>
          </div>
        ...
        <div className="shop-first-mobile" style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          ...
          <div>
            <h1 style={{ fontFamily: 'Poppins, sans-serif', fontWeight: 800, fontSize: 'clamp(22px, 3vw, 30px)', color: '#0a0a0a', letterSpacing: '-0.025em', marginBottom: 8 }}>
              Borraccia Inox Sport
            </h1>
            <p style={{ fontSize: '14px', color: '#666', lineHeight: 1.65 }}>
              Borraccia in acciaio inox 600ml a doppia parete. Area di stampa 22×14 cm. Mantiene le bevande fredde fino a 24h e calde fino a 12h.
            </p>
          </div>
          ...
          <div style={{ background: '#fff', border: '1px solid #e8e8e8', borderRadius: 16, padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ fontSize: '12px', color: '#888', lineHeight: 1.7, padding: '10px 12px', background: '#f9f9f9', borderRadius: 8 }}>
              <b style={{ color: '#555' }}>Borraccia Inox Sport 600ml</b> — area stampa 22×14 cm
            ...
            <p style={{ fontSize: '11px', color: '#bbb', textAlign: 'center' }}>
              Acciaio inox a doppia parete · 600ml · Ritiro in studio
            </p>
          </div>
        </div>
      </div>
