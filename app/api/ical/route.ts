...
export async function GET(req: Request) {
  try {
    ...
    const events = clienti.map(c => {
      ...
      const summary = escapeIcal(`${c.nome1 ?? 'Cliente'} — ${c.categoria}`)
      // data_evento può essere string "YYYY-MM-DD" o Date object
      const rawDate = c.data_evento instanceof Date
        ? c.data_evento.toISOString()
        : String(c.data_evento)
      ...
      const locationVal = c.luogo_evento?.trim()
      const descVal = c.note?.trim()
      ...
      return [
        ...
        dtstart,
        dtend,
        foldLine(`SUMMARY:${emoji} ${summary}`),
        locationVal ? foldLine(`LOCATION:${escapeIcal(locationVal)}`) : null,
        descVal ? foldLine(`DESCRIPTION:${escapeIcal(descVal)}`) : null,
        'END:VEVENT',
      ].filter(Boolean).join('\r\n')
    })
    ...
    return new NextResponse(`Errore interno: ${msg}`, { status: 500 })
  }
}
